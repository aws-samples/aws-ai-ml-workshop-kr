...
def main():
    ...
    # Test setting
    gradient_checkpointing_options = [True, False]
    gradient_accumulation_options = [5, 1]
    batch_size_options = [5, 1]
    ...
    if is_main_process():
        ...
        main_process_print(f"테스트 파라미터: gradient_checkpointing, gradient_accumulation_steps")
        main_process_print(f"{'=' * 70}")
        
    # 벤치마크 실행
    for gradient_checkpointing in gradient_checkpointing_options:
        for gradient_accumulation_steps in gradient_accumulation_options:
            for batch_size in batch_size_options:
                ...
                if is_main_process():
                    ...
                    main_process_print(f'  batch_size={batch_size}')

                result = benchmark_gradient_trick(
                    dataset=dataset,
                    batch_size=batch_size,
                    num_workers=num_workers,
                    pin_memory=pin_memory,
