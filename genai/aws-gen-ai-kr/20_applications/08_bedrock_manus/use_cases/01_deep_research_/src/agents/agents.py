...
#from src.tools.clarification_tools import clarification_tool_config, process_clarification_tool
...
class create_react_agent():
    ...
    def __init__(self, **kwargs):
        ...
        if self.agent_name == "clarifier": self.tool_config = clarification_tool_config
        elif self.agent_name == "researcher": self.tool_config = research_tool_config
            ...
            if response["stop_reason"] == "tool_use":
                ...
                for content in ai_message['content']:
                    if 'toolUse' in content:
                        ...
                        if self.agent_name == "clarifier": tool_result_message = process_clarification_tool(tool)
                        elif self.agent_name == "researcher": tool_result_message = process_search_tool(tool)
                        ...
                        if self.agent_name == "reporter":
                            logger.info(f"tool_result_message: {tool_result_message}")
