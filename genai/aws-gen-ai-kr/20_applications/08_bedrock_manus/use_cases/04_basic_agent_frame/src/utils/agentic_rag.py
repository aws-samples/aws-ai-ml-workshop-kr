...
class retriever_utils():
    ...
    bedrock_runtime_client = boto3.Session().client('bedrock-runtime')
    ...
    def get_num_tokens(cls, text):
        return cls.client.get_num_tokens(text)
    ...
    def get_rerank_docs(cls, **kwargs):
        ...
        for idx, (context, score) in enumerate(contexts):
            ...
            token_size = cls.get_num_tokens(query+page_content)
            
            exceed_flag = False
            if token_size > cls.reranker_token_limit:
                exceed_flag = True
                ...
                for splited_doc in splited_docs:
                    ...
                    length.append(cls.get_num_tokens(splited_doc.page_content))
        ...
        if kwargs["reranker_endpoint_name"] == "cohere-reranker-3-5":
            query = rerank_queries["inputs"][0]["text"]
            # Extract documents from text_pair of all inputs
            documents = [item["text_pair"] for item in rerank_queries["inputs"] if "text_pair" in item]
            
            # Create the request_body structure
            request_body = {
                "query": query,
                "documents": documents,
                "api_version": 2
            }
            rerank_queries = json.dumps(request_body)

            # Bedrock API 호출
            response = cls.bedrock_runtime_client.invoke_model(
                modelId="cohere.rerank-v3-5:0",
                body=rerank_queries
            )
            outs = json.loads(response['body'].read().decode()) ## for json
            #outs = sorted(outs["results"], key=lambda x: x["index"], reverse=False)

            outs = [{'index': item['index'], 'score': item['relevance_score']} for item in outs["results"]]
            outs = sorted(outs, key=lambda x: x['index'])
