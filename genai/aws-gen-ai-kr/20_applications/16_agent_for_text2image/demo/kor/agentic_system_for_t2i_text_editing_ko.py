...
class GraphState(TypedDict):
    ...
    origin_ask_repo: str
    ko_origin_ask_repo: str

    prompt_components: dict
    ko_prompt_components: dict

    image_prompt: dict
    ko_image_prompt: dict
    ...
    mask_image: str
    prev_node: str
    original_image: str # The target image path for text editing

    error: bool
    error_message: str


class GenAITextEditor():
    """A class for editing images."""
    def __init__(self, **kwargs):
        """Initialize the GenAIEditor with required models and components.
    ...
    def _body_generator(
            self, image_prompt, taskType="INPAINTING", maskImage=None,
            original_image=None,
    ):    
        ...
        Args:
            ...
            original_image: Optional original image for in/outpainting.
        ...
        if taskType == "INPAINTING":
            # Create request body for inpainting
            ...
            body_dict = {
                ...
                "inPaintingParams": {
                    ...
                    "maskPrompt": image_prompt["mask_prompt"],
            ...
            raise ValueError("유효하지 않은 taskType입니다. 'INPAINTING' 이여야 합니다.")
    ...
    def _graph_definition(self, **kwargs):
        ...
        def ask_reformulation(state):
            ...
            user_prompts = dedent(
                ...
                Here is mask_prompt: <mask_prompt>{mask_prompt}</mask_prompt>
                '''
            )
            context = {
                "ask": ask,
                ...
                "mask_prompt": image_prompt["mask_prompt"] if image_prompt != "None" else "None",
            }
            ...
            ko_ask_repo = self._translate_text(ask_repo)
            
            if origin_ask_repo == "None": 
                origin_ask_repo = ask_repo
                ko_origin_ask_repo = ko_ask_repo
            
            return self.state(
                ask_repo=ask_repo,
                ko_ask_repo=ko_ask_repo,
                origin_ask_repo=origin_ask_repo,
                ko_origin_ask_repo=ko_origin_ask_repo,
        ...
        def check_readness_prompt_generation(state):
            ...
            ko_prompt_components = {}
            ko_prompt_components = {
                key: {
                    "content": self._translate_text(value["content"]) if value["content"] else None
                }
            ...
            return self.state(
                ...
                ko_prompt_components=ko_prompt_components,
        ...
        def prompt_generation_for_image(state):
            ...
            system_prompts = dedent(
                '''
                <task>
                추출된 시각 요소들을 활용하여 이미지 에디팅 {image_model}에 최적화된 고품질 프롬프트를 생성하세요.
                </task>
            
                <instruction>
                이미지 에디팅 프롬프트 전문가로서, 다음 원칙에 따라 최적의 프롬프트를 구성하세요:
                ...
                2. {image_model} 최적화 전략:
                   ...
                   - stable-diffusion 최적화:
                     * 참조 아티스트나 스타일 명시
                     * "highly detailed", "masterpiece", "professional" 등의 품질 키워드
                     * 스타일 관련 구체적 키워드 활용
            ...
            ko_image_prompt = {}
            ko_image_prompt = {
                key: self._translate_text(value) if value else None
                for key, value in image_prompt.items()
            }


            return self.state(
                image_prompt=image_prompt, 
                ko_image_prompt=ko_image_prompt, 
                prev_node="PROMPT_GENERATION_FOR_IMAGE")
        ...
        def image_generation(state):
            ...
            task_type = state.get("task_type", "INPAITING")  # 추가: 기본값은 "INPAINTING"
    ...
    def invoke(self, **kwargs):
        ...
        inputs = self.state(
            ...
            task_type=kwargs.get("task_type", "INPAINTING"),  # 기본값은 "INPAINTING"
