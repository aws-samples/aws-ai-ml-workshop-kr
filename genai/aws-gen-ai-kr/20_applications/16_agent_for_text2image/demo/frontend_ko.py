...
import kor.agentic_system_for_t2i_stepwise_ko
import kor.agentic_system_for_t2i_text_editing_ko
import kor.agentic_system_for_t2i_inpainting_ko
import kor.agentic_system_for_t2i_outpainting_ko
...
def generate_stepwise_image(prompt, results, progress=gr.Progress()):
    ...
    analyzer_for_stepwise = kor.agentic_system_for_t2i_stepwise_ko.genai_analyzer( 
        llm=llm,
    ...
    for result in progress.tqdm(analyzer_for_stepwise.invoke(ask=dedent(prompt), image_model="stable-diffusion", file_path_name=generated_img_path)):
        if result["key"] == "ImageGeneration":
            time.sleep(10)
        results.append(result)
        yield results, "이미지 생성 완료"
...
def generate_text_editing_image(original_image, edit_count, prompt, results, progress=gr.Progress()):
    ...
    analyzer_for_text_editing = kor.agentic_system_for_t2i_text_editing_ko.GenAITextEditor(
        llm=llm,
...
def generate_mask_editing_image(original_image, 
    ...
    if task_type == "INPAINTING":
        GenAIClass = kor.agentic_system_for_t2i_inpainting_ko.GenAIInPainting
        file_name = f"{inpainting_edited_images_by_mask_path}/EDITED_IMAGE_{edit_count}.png"
    
    else:
        GenAIClass = kor.agentic_system_for_t2i_outpainting_ko.GenAIOutPainting
...
def format_reflection_output_to_html(data: Dict) -> str:
    ...
    if data.get("retouch") == "true" and data.get("should_regeneration") == "pass":
        html_content += """
                <p class="retry"><strong>Retry Count:</strong> 리터치가 필요하지만, 단계별 최대 재생성 횟수(2회)를 초과하여 다음 단계로 강제 진행됩니다.</p>
        """   
    else:
        html_content += f"""
    ...
    suggestions = data.get("ko_suggestions", [])
    if suggestions:
...
def format_prompt_reformulation_output_to_html(data: Dict) -> str:
    ...
    html_content += f"""
            <div class="section">
                ...
                <p class="control"><strong>Control Strength:</strong> {data.get("ko_prompt_repo", {}).get("control_strength", "N/A")}</p>
                <h3>Positive Prompt</h3>
                <div class="positive">
                    <p>{data.get("ko_prompt_repo", {}).get("positive", "No positive prompt available.")}</p>
                </div>
                <h3>Negative Prompt</h3>
                <div class="negative">
                    <p>{data.get("ko_prompt_repo", {}).get("negative", "No negative prompt available.")}</p>
                </div>
            </div>
        </div>
    </body>
...
def format_ask_reformulation_output_to_html(data: Dict) -> str:
    ...
    html_content +=f"""
            <div class="section">
                ...
                <div class="content">
                    <p>{data.get("ko_origin_ask_repo", "No origin prompt available.")}</p>
                </div>
                <h3>Reformed Prompt</h3>
                <div class="content">
                    <p>{data.get("ko_ask_repo", "No reformed prompt available.")}</p>
                </div>
            </div>
        </div>
    </body>
...
def format_check_readness_prompt_generation_output_to_html(data: Dict) -> str:
    ...
    html_content +=f"""
            <div class="section">
                <h3>Subject</h3>
                <div class="content">
                    <p>{data.get("ko_prompt_components", {}).get("subject", {}).get("content", "None")}</p>
                </div>
                <h3>Action</h3>
                <div class="content">
                    <p>{data.get("ko_prompt_components", {}).get("action", {}).get("content", "None")}</p>
                </div>
                <h3>Environment</h3>
                <div class="content">
                    <p>{data.get("ko_prompt_components", {}).get("environment", {}).get("content", "None")}</p>
                </div>
                <h3>Lighting</h3>
                <div class="content">
                    <p>{data.get("ko_prompt_components", {}).get("lighting", {}).get("content", "None")}</p>
                </div>
                <h3>Style</h3>
                <div class="content">
                    <p>{data.get("ko_prompt_components", {}).get("style", {}).get("content", "None")}</p>
                </div>
                <h3>Camera Position</h3>
                <div class="content">
                    <p>{data.get("ko_prompt_components", {}).get("camera_position", {}).get("content", "None")}</p>
                </div>
            </div>
        </div>
    </body>
...
def format_prompt_generation_for_image_output_to_html(data: Dict) -> str:
    ...
    html_content +=f"""
            <div class="section">
                <h3>Main Prompt</h3>
                <div class="content">
                    <p>{data.get("ko_image_prompt", {}).get("main_prompt", "No main prompt available.")}</p>
                </div>
                <h3>Negative Prompt</h3>
                <div class="negative">
                    <p>{data.get("ko_image_prompt", {}).get("negative_prompt", "No negative prompt available.")}</p>
                </div>
                <h3>Mask Prompt</h3>
                <div class="highlight">
                    <p>{data.get("ko_image_prompt", {}).get("mask_prompt", "사용자가 이미지 마스크를 직접 제공하였기 때문에, mask prompt는 모델에 의해 생성되지 않았습니다.")}</p>
                </div>
            </div>
        </div>
    </body>
...
html_content_for_notice = """
    <div class="lightmode-notice">
        <span class="icon">⚠️</span>
        <span class="text">
            최적의 데모 경험을 위해 브라우저를 <strong>라이트 모드(Light Mode)</strong>로 설정해 주세요.
        </span>
    </div>
    ...
    <div class="translation-notice">
        ...
        <span class="text">
            내부 모델들의 입력과 출력은 <strong>영어</strong>로 처리되며, 사용자의 이해를 돕기 위해 <strong>한국어로 번역</strong>하여 보여드립니다.
        </span>
    </div>
...
with gr.Blocks(fill_height=True) as demo:
    ...
    with gr.Tab("1️⃣ Generate Image"):
        ...
        html_notice = gr.HTML(html_content_for_notice)
    ...
    with gr.Tab("3️⃣ Edit Image By Mask"):
        ...
        with gr.Row(): # Inputs
            original_image = gr.ImageMask(label="Base Image", show_label=True)
            with gr.Column():
